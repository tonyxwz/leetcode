...
public:
    ...
    void pathSum(TreeNode* root, int sum, vector<int>& current_path, vector<vector<int>>& ans) {
        if (!root) return;
        current_path.push_back(root->val);
        if (!root->left && !root->right && root->val == sum) {
            // vector::push_back copy (or move) the new elements. so further
            // operation on current_path won't affect those paths already pushed to ans
