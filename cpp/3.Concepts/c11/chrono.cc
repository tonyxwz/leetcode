 ...
 * clocks:
 *
 * std::chrono::system_clock:  current time according to the system (it is not
 steady)
 ...
 *    duration<int, ratio<1,1>> --  number of seconds stored in a int  (this is
 * the default) duration<double, ration<60,1>> -- number of minutes (60 seconds)
 * stored in a double convenince duration typedefs in the library: nanoseconds,
 * microseconds, milliseconds, seconds, minutes, hours system_clock::duration --
 * duration<T, system_clock::period> T is a signed arithmetic type, could be int
 * or long or others
 */
chrono::microseconds mi(2745);
chrono::nanoseconds na = mi;
chrono::milliseconds mill = chrono::duration_cast<chrono::milliseconds>(
  mi);          // when information loss could happen, convert explicitly
...
*          00:00 January 1, 1970 (Corordinated Universal Time - UTC)  -- epoch
of a clock
