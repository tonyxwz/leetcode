/*
Regular Expression:  a regular expression is a specific pattern that provides
concise and flexible means to "match" strings of text, such as particular
characters, words, or patterns of characters. - wikipedia
*/
...
main()
{
  ...
  while (true) {
    ...
    // newline regex e("abc?");               // ?       Zero or 1 preceding
    // character regex e("abc*");               // *       Zero or more preceding
    // character regex e("abc+");               // +       One of more preceding
    // character regex e("ab[cd]*");            // [...]   Any character inside
    // the square brackets regex e("ab[^cd]*");           // [...]   Any
    // character not inside the square brackets regex e("ab[cd]{3,5}"); regex
    // e("abc|de[\]fg]");         // |       Or regex  e("(abc)de+\\1");       //
    // \1      First group regex  e("(ab)c(de+)\\2\\1"); regex
    // e("[[:w:]]+@[[:w:]]+\.com"); // [[:w:]] word character: digit, number, or
    // underscore
...
main()
{
  ...
  // regex e("[[:punct:]]+");  // Printable character that is not space, digit,
  // or letter. regex e("[ [:punct:]]+");
  regex e("([[:w:]]+)@([[:w:]]+)\.com");
