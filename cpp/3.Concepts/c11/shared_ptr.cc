...
public:
  ...
  // void enter(DogHouse* h) { h->setDog(shared_from_this()); }  // Dont's call
...
public:
  void setDog(shared_ptr<Dog> p)
  {
    m_pD = p;
    cout << "Dog entered house." << endl;
  }
};
...
main()
{
  ...
  // auto pD2 = make_shared<Dog>( Dog("Smokey") ); // Don't use shared pointer
  // for object on stack.
  ...
  /********** Custom Deleter ************/
  shared_ptr<Dog> pD4(new Dog("Victor"), [](Dog* p) {
    cout << "deleting a dog.\n";
    delete p;
  });
